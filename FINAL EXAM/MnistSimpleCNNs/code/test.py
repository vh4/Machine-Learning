...
def run(p_seed=0, p_kernel_size=5, p_logdir="temp"):
    ...
    model1.load_state_dict(torch.load("/content/logs/%s/model%03d.pth"%(p_logdir,p_seed)))
    ...
    np.savetxt("/content/logs/%s/wrong%03d.txt"%(p_logdir,p_seed), wrong_images, fmt="%d")
